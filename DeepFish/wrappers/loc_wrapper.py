...
class LocWrapper(torch.nn.Module):
	...
	def val_on_batch(self, batch):
		...
		return {'val_samples': images.shape[0], 'val_loc': abs(float((np.unique(blobs) != 0).sum() - (points != 0).sum()))}
...
def compute_fp_loss(S_log, blob_dict):
	...
	for b in blob_dict["blobList"]:
		...
		loss += scale * F.nll_loss(S_log, torch.LongTensor(T).cuda()[None], ignore_index=1, reduction='mean')
...
def compute_bg_loss(S_log, bg_mask):
	loss = F.nll_loss(S_log, torch.LongTensor(bg_mask).cuda()[None], ignore_index=1, reduction='mean')
	return loss
...
def compute_split_loss(S_log, S, points, blob_dict):
	...
	for b in blob_dict["blobList"]:
		...
		scale = b["n_points"] + 1
		loss += float(scale) * F.nll_loss(S_log, torch.LongTensor(T).cuda()[None], ignore_index=1, reduction='mean')
...
def get_blob_dict_base(model, blobs, points, training=False):
	...
	for l in range(blobs.shape[0]):
		...
		for u in uniques:
			blobList += [{"class": l, "label": u, "n_points": 0, "size": 0, "pointsList": []}]
			n_fp += 1
